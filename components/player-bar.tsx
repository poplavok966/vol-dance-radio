...
export function PlayerBar() {
  ...
  return (
    <div className="fixed bottom-0 left-0 z-[1000] w-full border-t border-border bg-background/95 backdrop-blur-xl">
      <div className="mx-auto flex h-[84px] max-w-6xl items-center justify-between gap-4 px-4 md:px-8">
        {/* Left: play + meta */}
        <div className="flex min-w-0 flex-1 items-center gap-3.5 md:flex-none md:min-w-[280px]">
          <button
            onClick={toggle}
            aria-label={isPlaying ? 'Пауза' : 'Слухати'}
            className="grid size-12 shrink-0 place-items-center rounded-full bg-brand text-white shadow-[0_0_18px_rgba(255,85,0,0.5)] transition-transform hover:scale-105"
          >
            {isBuffering ? (
          ...
          <div className="min-w-0">
            <div className="text-[10px] font-semibold uppercase tracking-[0.2em] text-dim">
              VOL DANCE Radio
            </div>
        ...
        {/* Center: equalizer */}
        <div
          className={`hidden h-6 items-end gap-1 md:flex ${
            isPlaying ? 'vd-eq-playing' : ''
          }`}
          aria-hidden
        >
